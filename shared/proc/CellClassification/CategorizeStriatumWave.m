...
%   (altnerate: 'Sirota', i.e. as in Karalis & Sirota, 2018) note:
%   currently only uses valley-to-peak time and not asymmetry index
%
% OUTPUTS:
%
...
for iC = nCells:-1:1
    ...
    [pw(iC),vw(iC),asy(iC),v2p(iC)] = waveWidths(mWV);
    wv(iC,:) = mWV(:);
    cq.ampl(iC) = max(mWV(:));
...
switch cfg.cMethod
    ...
    case 'Sirota'
        ...
        fsi = find(v2p < .2 & fr > 2 & pw < 0.15 & cq.id >= cfg.MinID & cq.lr <= cfg.MaxLr & cq.ampl >= cfg.MinAmpl);
        msn = find(v2p > .2 & fr < 5 & cq.id >= cfg.MinID & cq.lr <= cfg.MaxLr & cq.ampl >= cfg.MinAmpl);
        other = setdiff(1:nCells,fsi);
        other = setdiff(other,msn);
